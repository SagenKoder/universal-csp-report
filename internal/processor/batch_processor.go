...
func New(cfg config.BatchProcessorConfig, storage storage.Storage, logger *logrus.Logger) *BatchProcessor {
	ctx, cancel := context.WithCancel(context.Background())
	
	reportChan := make(chan *models.CSPReport, cfg.QueueSize)
	batchChan := make(chan []*models.CSPReport, cfg.WorkerCount*2)
